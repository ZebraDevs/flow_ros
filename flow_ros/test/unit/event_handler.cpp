...
TEST(EventHandlerSingleThreaded, GetSubscribers)
{
  ...
  const auto callback = [](const EventHandler::Input& inputs) { return std::make_shared<TestMessage1>(); };
...
TEST(EventHandlerSingleThreaded, SingleInputNoOutputs)
{
  ...
  // SISO event handler type
  using EventHandler = flow_ros::
    EventHandler<std::tuple<>, std::tuple<flow_ros::Subscriber<TestMessage1, flow::driver::Next, flow::NoLock>>>;

  // Publisher to send message
  flow_ros::Publisher<TestMessage1> input_pub{router, "input", 1};

  // Dummy callback; returns 1 value
  const auto callback = [](const EventHandler::Input& inputs) -> std::tuple<> { return std::tuple<>{}; };
...
TEST(EventHandlerSingleThreaded, SingleInputSingleOutput)
{
  ...
  auto output_sub =
    router.subscribe<TestMessage1>("output", 1, [&output_msg](const TestMessage1::ConstPtr& msg) { output_msg = msg; });

  // Dummy callback; returns 1 value
  const auto callback = [](const EventHandler::Input& inputs) -> TestMessage1::Ptr {
...
TEST(EventHandlerSingleThreaded, SingleInputMultiOutput)
{
  ...
  // SISO event handler type
  using EventHandler = flow_ros::EventHandler<
    std::tuple<flow_ros::Publisher<TestMessage1>, flow_ros::Publisher<TestMessage2>>,
    std::tuple<flow_ros::Subscriber<TestMessage1, flow::driver::Next, flow::NoLock>>>;
  ...
  auto output_sub_2 = router.subscribe<TestMessage2>(
    "output_2", 1, [&output_msg_2](const TestMessage2::ConstPtr& msg) { output_msg_2 = msg; });
  ...
  std::shared_ptr<flow_ros::EventHandlerBase> handler = std::make_shared<EventHandler>(
    callback,
    std::make_tuple(
      std::make_shared<flow_ros::Publisher<TestMessage1>>(router, "output_1", 1),
